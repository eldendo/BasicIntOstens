...
var line: string;
	...
	begin
		...
		else fatal('TIS VERKEIRD')
	    end;
	    if MidStr(line,lp,5) <> 'TONNE' then fatal('TIS VERKEIRD');
	    setCh(lp+5);
	    if relop then begin line := trim(rightStr(line,length(line)-lp+1)) ;execLine end 
	end;
	
    begin
	if      leftStr(line,10) = 'SCHRIEFTUP' then begin setCh(11); expr_list end
	else if leftstr(line,6) = 'GONOAR' then begin setCh(7); lc := num_expression-1 end	
	else if leftstr(line,5) = 'STELT' then begin setCh(6); let; end
	else if leftstr(line,3) = 'OLS' then begin setCh(4); ifThen; end	
	else if leftstr(line,10) = 'GODEKINOAR' then begin setCh(11); push(lc); lc := num_expression-1 end
	else if leftstr(line,12) = 'KEIRDEKIWERE' then lc := pop		
	else if leftstr(line,5) = 'LEEST' then begin setCh(6);varList end	
	else if leftstr(line,10) = 'UPMERKINGE' then // do nothing
	else if leftStr(line,10) = 'LOTMOLOPEN' then run
	else if leftstr(line,7) = 'UTVAGEN' then clear	
	else if leftstr(line,8) = 'TOOGDEKI' then list
	else if leftstr(line,7) = 'UPNIEUW' then new
	else if leftstr(line,10) = 'STOPTERMEE' then lc := lastLine
	else if leftstr(line,4) = 'LOAN' then load
	else if leftstr(line,7) = 'UPSLOAN' then save
	else if leftstr(line,9) = 'TRAPPETOF' then begin writeln('tjow'); terminated := true; end
	else fatal('TIS VERKEIRD') 
    end;
...
begin // main
    writeln('+-----------------------------------------+');
    writeln('| Basic in ''t ostens                      |');
    writeln('| deur ir. Marc Dendooven                 |');
    writeln('+-----------------------------------------+');
    writeln;
    new; 
    clear;
